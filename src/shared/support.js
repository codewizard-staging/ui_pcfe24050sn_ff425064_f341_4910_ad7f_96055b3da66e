import {
   ...
   SetPetServiceSingle, SetPetOwnerSingle, SetPetOwnerPetsJoin, SetPetCareCenterSingle, SetPetCareCenterImagesJoin, SetPetCareCenterPetsJoin, SetDocumentSingleMedia, SetPetSingle
} from "./services";
...
const numberItems_PetOwner = [  'OwnerId' ];
    return new Promise(async (resolve) => {
        ...
        let rslt = await SetPetOwnerPetsJoin(data);
        global.Busy(false);
        if (rslt.status) {
            id = rslt.id;
            status = true;
...
const numberItems_PetCareCenter = [  'PcId','Latitude','Longitude','Pincode' ];
    return new Promise(async (resolve) => {
        ...
        let rslt = await SetPetCareCenterPetsJoin(data);
        global.Busy(false);
        if (rslt.status) {
            id = rslt.id;
            status = true;
